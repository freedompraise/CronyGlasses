...
def create_test_drink(name="Drink 1", price=None, description=None):
    ...
    return Drink.objects.create(name=name, price=price, description=description, image=drink_image)
...
def create_multiple_test_order_items(number_of_order_items, order, drink, quantity):
    ...
    for i in range(number_of_order_items):
        order_items.append(create_test_order_item(order=order, drink=drink, quantity=quantity))
    return order_items